  ...
  try {
    ...
    if (minimumCredits < requiredCredits) {
      const newInvoice = new EInvoice({
        userid,
        status:
          "Insufficient credits. Please check your credit balance or contact admin.",
        ...invoiceData,
      });
    ...
    const existingInvoice = await EInvoice.findOne({
      "document_details.document_number":
        invoiceData.document_details.document_number,
      status: "Success",
    ...
    if (existingInvoice) {
      return res
        ...
        .json({
          message:
            "E-Invoice with this document number has already been generated successfully.",
        });
    }
    ...
    if (apiStatus) {
      ...
      const errorMsg =
        ...
        "E-Invoice creation failed due to unknown reasons.";
    ...
    res
      .status(500)
      .json({ message: "Error creating E-Invoice", error: error.message });
  }
};

exports.cancelEinvoice = async (req, res) => {
  try {
    const { access_token, user_gstin, irn, cancel_reason, cancel_remarks } =
      req.body;
    ...
    const response = await axios.post(
      "https://clientbasic.mastersindia.co/cancelEinvoice",
      postData
    );
  ...
  try {
    ...
    if (!invoices.length) {
      return res
        ...
        .json({
          message:
            "No invoices found for this client within the specified date range",
        });
    }
