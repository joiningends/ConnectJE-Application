...
exports.sendBulkWhatsAppMessages = async (req, res) => {
  ...
  try {
    ...
    const recipientNumbers = await RecipientNumber.find({
      section: sectionId,
    }).distinct("contactNumbers");
    ...
    const sendMessages = async () => {
      for (const recipientNumber of recipientNumbers) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            failureCount++;
            console.error(
              `Failed to send message to ${recipientNumber}: ${response.statusText}`
            );
          }
        } catch (error) {
          failureCount++;
          console.error(
            `Error sending message to ${recipientNumber}: ${error}`
          );
        }
        ...
        const trackingServiceUrl =
          "http://localhost:5001/api/v1/groups/track-message";
        try {
      ...
      console.log(
        `Cycle completed. Success: ${successCount}, Failure: ${failureCount}`
      );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to send bulk WhatsApp messages" });
  }
};
  ...
  try {
    ...
    for (const recipientNumber of recipientNumbers) {
      ...
      fetch(trackingServiceUrl, {
        method: "POST",
        body: JSON.stringify({
          clientId: section.user._id,
          sender: section.user.name,
          recipient: recipientNumber,
    ...
    res.status(500).json({
      success: false,
      error: "Failed to send bulk WhatsApp media messages",
    });
  }
};
...
exports.createInstance = async (req, res) => {
  ...
  try {
    const response = await axios.post(
      "http://hisocial.in/api/create_instance",
      null,
      {
        params: {
          access_token: access_token,
...
exports.getQRCode = async (req, res) => {
  ...
  try {
    const response = await axios.post(
      "http://hisocial.in/api/get_qrcode",
      null,
      {
        params: {
          instance_id: instance_id,
  ...
  try {
    ...
    if (minimum < counts) {
      ...
      return res.status(400).json({
        success: false,
        message: "Not enough message templates available.",
      });
    }
    ...
    for (let i = 0; i < recipientNumbers.length; i++) {
      ...
      for (const contact of recipient.contactNumbers) {
        if (cancellationTokens[campainid]) {
          console.log(
            `Bulk message sending for campaign ${campainid} was cancelled.`
          );
...
exports.sendBulkWhatsAppMessagess = async (req, res) => {
  ...
  try {
    ...
    if (!scheduleDate.isValid()) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid schedule time format." });
    }
    ...
    console.log(
      `Campaign scheduled to start in ${delay} ms (${delay / 1000}s).`
    );
    ...
    res.json({
      ...
      message:
        delay === 0
          ...
          : `Campaign scheduled for ${scheduleDate.format(
              "YYYY-MM-DD HH:mm:ss"
            )} successfully.`,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to schedule campaign." });
  }
};
...
exports.sendBulkWhatsAppMessagesMedia = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "Scheduled time in Asia/Kolkata:",
    ...
    console.log(
      `Media campaign scheduled to start in ${delay} ms (${delay / 1000}s).`
    );
    ...
    res.json({
      ...
      message:
        delay === 0
          ...
          : `Media campaign scheduled for ${scheduleDate.format(
              "YYYY-MM-DD HH:mm:ss"
            )} successfully.`,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to schedule media campaign." });
  }
};

exports.resendTextMessageToGroup = async (req, res) => {
  const access_token = "64da53e6c44e5";
  ...
  try {
    ...
    if (!Array.isArray(groups) || groups.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "No groups provided." });
    }
    ...
    if (minimumCredits < totalCost) {
      await trackMessageFailure(
        ...
        "No",
        "Failed",
        "Please check your credit is low or contact admin"
      );
      return res
        .status(400)
    ...
    if (scheduleTime) {
      ...
      console.log(
        "Schedule time in Asia/Kolkata:",
        scheduleDate.format("YYYY-MM-DD HH:mm:ss")
      );

      if (scheduleDate.isValid() && scheduleDate.isAfter(now)) {
        // Schedule the message at the given time using cron
        const cronExpression = `${scheduleDate.minutes()} ${scheduleDate.hours()} ${scheduleDate.date()} ${
          scheduleDate.month() + 1
        } *`;

        nodeCron.schedule(cronExpression, async () => {
          console.log(
            `Scheduled campaign started at ${scheduleDate.format(
              "YYYY-MM-DD HH:mm:ss"
            )}`
          );
        ...
        return res.json({
          success: true,
          message: `Campaign scheduled to start at ${scheduleDate.format(
            "YYYY-MM-DD HH:mm:ss"
          )}`,
        });
      } else {
        return res.status(400).json({
          success: false,
          message: "Invalid or past schedule time provided.",
        });
      }
    }
...
async function resendMessagesToGroups(
  user,
  groups,
  message,
  instance_id,
    ...
    while (attempt < maxRetries && !messageSent) {
      ...
      try {
        ...
        if (attempt >= maxRetries) {
          ...
          failedGroups.push({
            groupId,
            message: "Message sending failed after retries",
          });
        } else {
          console.log(
            `Retrying message to group ${group.name} (attempt ${attempt})`
          );
        }
      }
    }
...
async function trackMessageFailure(
  clientId,
  sender,
  attachment,
  status,
...
exports.resendMediaMessageToGroup = async (req, res) => {
  const access_token = "64da53e6c44e5";
  const hisocialWhatsAppGroupEndpoint = "https://hisocial.in/api/send_group";
  const trackingServiceUrl =
    "http://localhost:5001/api/v1/groups/track-message/excel";
  ...
  try {
    ...
    if (message && media_url) {
      ...
      totalCost =
        groups.length *
        (user.whatsappmodelgroup.cmf + user.whatsappmodelgroup.cm);
    } else if (media_url) {
      // Only media
      ...
      return res.status(400).json({
        success: false,
        message: "Either message or media URL must be provided.",
      });
    }
    ...
    if (scheduleTime) {
      ...
      if (scheduleDate.isValid() && scheduleDate.isAfter(now)) {
        ...
        nodeCron.schedule(cronExpression, async () => {
          console.log(
            `Scheduled media campaign started at ${scheduleDate.format(
              "YYYY-MM-DD HH:mm:ss"
            )}`
          );
          await sendMediaMessagesToGroups(
            user,
            groups,
            message,
            instance_id,
        ...
        return res.json({
          success: true,
          message: `Media campaign scheduled to start at ${scheduleDate.format(
            "YYYY-MM-DD HH:mm:ss"
          )}`,
        });
      } else {
...
async function sendMediaMessagesToGroups(
  user,
  groups,
  message,
  instance_id,
  ...
  for (let groupId of groups) {
    ...
    while (attempt < maxRetries && !messageSent) {
      ...
      try {
        ...
        if (attempt >= maxRetries) {
          console.error(
            "Error sending media message to WhatsApp group:",
            error
          );
          const remark =
            "Please check if Instance ID is active or contact admin";
          ...
          failedGroups.push({
            groupId,
            message: "Media message sending failed after retries",
          });
        } else {
          console.log(
            `Retrying media message to group ${group.name} (attempt ${attempt})`
          );
        }
      }
    }
...
exports.downloadexcel = async (req, res) => {
  ...
  try {
    ...
    if (!messages || messages.length === 0) {
      return res
        .status(404)
        .send("No messages found for the specified campaign ID.");
    }
...
exports.resendBulkWhatsAppMessagesapi = async (req, res) => {
  const access_token = "64da53e6c44e5";
  const hisocialWhatsAppEndpoint = "https://hisocial.in/api/send";
  const trackingServiceUrl =
    "http://localhost:5001/api/v1/groups/track-message/excel";
      ...
      while (attempt < maxRetries && !messageSent) {
        ...
        try {
          ...
          if (attempt >= maxRetries) {
            ...
            console.log(
              `Retrying message to ${recipientNumber} (attempt ${attempt})`
            );
          }
        }
...
exports.resendBulkWhatsAppMessagesMediaapi = async (req, res) => {
  const access_token = "64da53e6c44e5";
  const hisocialWhatsAppEndpoint = "https://hisocial.in/api/send";
  const trackingServiceUrl =
    "http://localhost:5001/api/v1/groups/track-message/excel";
  ...
  try {
    ...
    if (message) {
      requiredCredits =
        numbers.length * (client.whatsappmodel.cmf + client.whatsappmodel.cm);
    } else {
      ...
      while (attempt < maxRetries && !messageSent) {
        ...
        try {
          ...
          if (status === "success") {
            ...
            if (message) {
              client.Totalcredit -=
                client.whatsappmodel.cmf + client.whatsappmodel.cm;
              client.creditused +=
                client.whatsappmodel.cmf + client.whatsappmodel.cm;
            } else {
...
exports.resendSingleWhatsAppMessageapi = async (req, res) => {
  const access_token = "64da53e6c44e5";
  const hisocialWhatsAppEndpoint = "https://hisocial.in/api/send";
  const trackingServiceUrl =
    "http://localhost:5001/api/v1/groups/track-message/excel";
...
exports.resendSingleWhatsAppMessageMediaapi = async (req, res) => {
  const access_token = "64da53e6c44e5";
  const hisocialWhatsAppEndpoint = "https://hisocial.in/api/send";
  const trackingServiceUrl =
    "http://localhost:5001/api/v1/groups/track-message/excel";
  ...
  try {
    const { instance_id, clientid, number, message, filename, media_url } =
      req.body;
    ...
    const minimumCredits = client.Totalcredit;
    let requiredCredits = message
      ? client.whatsappmodel.cmf + client.whatsappmodel.cm
      : client.whatsappmodel.cmf;
  ...
  try {
    ...
    if (fromDate && toDate) {
      ...
      if (isNaN(fromISO.getTime()) || isNaN(toISO.getTime())) {
        return res
          .status(400)
          .send("Invalid date format. Please use DD/MM/YYYY.");
      }
    ...
    if (!messages || messages.length === 0) {
      return res
        .status(404)
        .send("No messages found for the specified client and date range.");
    }
