...
exports.createPrescription = async (req, res) => {
  try {
    ...
    const pdfPath = await generateEyePrescriptionPDF({
      name: user.participantFields.find(field => field.fieldName === "Name")
        .fieldValue,
      phone: user.participantFields.find(
    ...
    res
      .status(201)
      .json({ message: "Prescription saved successfully!", pdfPath });
    ...
    (async () => {
      try {
        const emailHtml = ` 
        <p>Dear ${
          user.participantFields.find(field => field.fieldName === "Name")
            .fieldValue
        },</p>
        <p>Your eye prescription has been generated. Please find it attached to this email.</p>
        <p>If you have any questions, feel free to contact us.</p>
        <p>Best regards,<br>Your Team</p>
        <p style="margin-top: 20px; font-size: 12px; color: gray; text-align: center;">Powered by JoiningEnds</p>
      `;

        const email = user.participantFields.find(
        ...
        const mobileNumber = user.participantFields.find(
          field => field.fieldName === "MobileNumber"
        ).fieldValue;
        const mediaUrl = `http://localhost:5001/public/uploads/${pdfPath
          .split("/")
          .pop()}`;
        const message = `Dear ${
          user.participantFields.find(field => field.fieldName === "Name")
        ...
        await axios.post("http://localhost:5001/api/v1/wa/api/media/single", {
    ...
    return res
      .status(500)
      .json({ error: "Failed to save prescription and send notifications" });
  }
};
...
exports.getPrescriptionsByEventId = async (req, res) => {
  try {
    ...
    if (prescriptions.length === 0) {
      return res
        .status(404)
        .json({ message: "No prescriptions found for this event." });
    }
...
function generateEyePrescriptionPDF(data) {
  return new Promise((resolve, reject) => {
    ...
    doc
      .image(adminLogoPath, 40, 40, { fit: [adminLogoWidth, adminLogoHeight] })
      .moveDown(1);
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Eye Prescription", { align: "center" })
      .moveDown(0.5);
    ...
    doc
      .font("Helvetica")
      .fontSize(12)
      .text(data.name || "Auto Populated", 150, doc.y - 12);

    const optomaticsTextTop = doc.y - 10;
    doc
      .font("Helvetica-Bold")
      .text("Optometrist Name:", 300, optomaticsTextTop);
    doc
      .font("Helvetica")
      .text(data.partnername || "N/A", 410, optomaticsTextTop);
    ...
    // Table Headers
    const rowHeight = 30;
    const columnWidth = (boxWidth - 80) / 4; // Adjust column width
    doc.fontSize(12).font("Helvetica-Bold");

    doc.text("Spherical", 80, boxTop + 5, {
      width: columnWidth,
      align: "center",
    });
    doc.text("Cylindrical", 80 + columnWidth, boxTop + 5, {
      width: columnWidth,
      align: "center",
    });
    doc.text("Axis", 80 + columnWidth * 2, boxTop + 5, {
      width: columnWidth,
      align: "center",
    });
    doc.text("Additional", 80 + columnWidth * 3, boxTop + 5, {
      width: columnWidth,
      align: "center",
    });

    // Left Eye row
    doc.font("Helvetica-Bold").text("Left Eye", 40, boxTop + rowHeight + 5);
    doc.font("Helvetica").fontSize(12);

    doc.text(data.leftEye.spherical || "N/A", 80, boxTop + rowHeight + 5, {
      width: columnWidth,
      align: "center",
    });
    doc.text(
      data.leftEye.cylindrical || "N/A",
      80 + columnWidth,
      boxTop + rowHeight + 5,
      { width: columnWidth, align: "center" }
    );
    doc.text(
      data.leftEye.axis || "N/A",
      80 + columnWidth * 2,
      boxTop + rowHeight + 5,
      { width: columnWidth, align: "center" }
    );
    doc.text(
      data.leftEye.additional || "N/A",
      80 + columnWidth * 3,
      boxTop + rowHeight + 5,
      { width: columnWidth, align: "center" }
    );
    ...
    doc
      .font("Helvetica-Bold")
      .text("Right Eye", 40, boxTop + rowHeight * 2 + 5);
    doc.font("Helvetica").fontSize(12);

    doc.text(data.rightEye.spherical || "N/A", 80, boxTop + rowHeight * 2 + 5, {
      width: columnWidth,
      align: "center",
    });
    doc.text(
      data.rightEye.cylindrical || "N/A",
      80 + columnWidth,
      boxTop + rowHeight * 2 + 5,
      { width: columnWidth, align: "center" }
    );
    doc.text(
      data.rightEye.axis || "N/A",
      80 + columnWidth * 2,
      boxTop + rowHeight * 2 + 5,
      { width: columnWidth, align: "center" }
    );
    doc.text(
      data.rightEye.additional || "N/A",
      80 + columnWidth * 3,
      boxTop + rowHeight * 2 + 5,
      { width: columnWidth, align: "center" }
    );
    ...
    const partnerBoxTop = boxTop + boxHeight + 20;
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("In Association with:", 50, partnerBoxTop);
    ...
    doc.image(partnerLogoPath, 50, partnerBoxTop + 25, {
      fit: [partnerLogoWidth, partnerLogoHeight],
    });

    // Partner Name and Phone
    doc.moveDown(2.5);
    doc.font("Helvetica-Bold").text("Partner Name:", 50, doc.y);
    doc.font("Helvetica").text(data.partnername || "N/A", 150, doc.y - 12);

    doc.moveDown(0.5);
    doc.font("Helvetica-Bold").text("Phone:", 50, doc.y);
    doc
      .font("Helvetica")
      .text(data.partnermobilenumber || "N/A", 150, doc.y - 12);
...
exports.downloadPrescriptionPDF = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to download the prescription PDF." });
  }
};
=======
const EyePrescription = require("../models/eyePrescription");
