...
exports.getTotalSentMessages = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to retrieve total sent messages count" });
  }
};
...
exports.getMessagesByDateRange = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to retrieve messages by date range" });
  }
};
...
exports.updateTotalCredit = async (req, res) => {
  ...
  try {
    ...
    const newTotalCredit = client.totalcredit.reduce(
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to update total credit" });
  }
};
...
exports.updateTotalCrediteinvoice = async (req, res) => {
  ...
  try {
    ...
    // Calculate the new Totalcredit value
    const newTotalCredit = client.totalcreditforeinvoice.reduce(
      (acc, entry) => acc + entry.value,
      0
    );
  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <p>Thanks,<br>Team JoiningEnds</p>`,
    };
    ...
    res
      .status(200)
      .json({ message: `${userType} password reset successfully` });
  } catch (error) {
