...
exports.registerForEvent = async (req, res) => {
  try {
    const { eventId, participantData, selectedTimeslot, slotId } = req.body;
    ...
    if (event.eventType === "1" || event.eventType === "2") {
      ...
      const slotData = await Slot.findById({ _id: slotId });
      if(!slotData){
      ...
      // const isTimeslotValid = event.timeslot.some(
      ...
      // ) {
      //   return res.status(400).json({
      //     message: `Maximum registrations reached for timeslot: ${date} ${time}`,
      //   });
      // }
      ...
      if (existingRegistrations >= event.maxRegistrations) {
        return res
          .status(400)
          .json({ message: "Maximum registrations reached for the event" });
      }
    }
    ...
    // -------------------------Update the Slot table to include the registration ID---------------------------
    await Slot.findByIdAndUpdate(
      slotId,
      { $push: { registeredUsers: newRegistration._id } },
    ...
    const emailSubject = htmlToText(
      event.emailSubject || "Event Registration Confirmation"
    );
...
exports.scanQRCode = async (req, res) => {
  ...
  try {
    ...
    const isScannerAuthorized = event.scanner.some(
      scanner => scanner.toString() === scannerId
    );

    if (!isScannerAuthorized) {
      return res
        .status(403)
        .send("You are not authorized to scan for this event.");
    }

    function parseDDMMYYYY(dateStr) {
      const [day, month, year] = dateStr.split("-").map(Number);
      // Create the date in local time (system time zone)
      return new Date(year, month - 1, day); // Month is 0-based
    }

    const currentDate = new Date(); // Current date
    const eventEndDate = parseDDMMYYYY(event.endDate); // Parse event.endDate correctly

    console.log("current date:", currentDate.toLocaleDateString());
    console.log("parsed end date:", eventEndDate.toLocaleDateString());
    ...
    if (normalizedCurrentDate > normalizedEventEndDate) {
      return res
        .status(400)
        .send("QR code has expired. The event has already ended.");
    }
...
exports.scanQRCodeop = async (req, res) => {
  ...
  try {
    ...
    if (registration.qrcodescannedbyop) {
      return res
        .status(400)
        .send("QR code has already been scanned by the operator.");
    }
    ...
    const isoptimisticAuthorized = event.optimistic.some(
      optimistic => optimistic.toString() === opId
    );
    ...
    return res
      .status(200)
      .send("QR code scanned successfully by the operator.");
  } catch (error) {
    console.error("Error scanning QR code:", error);
    res.status(500).send("Server error.");
  }
...
exports.getScannedCustomersbyscanner = async (req, res) => {
  ...
  try {
    ...
    if (scannedCustomers.length === 0) {
      return res
        .status(404)
        .send("No customers have scanned their QR codes for this event.");
    }
...
exports.getScannedCustomersbyop = async (req, res) => {
  ...
  try {
    ...
    if (scannedCustomers.length === 0) {
      return res
        .status(404)
        .send("No customers have been scanned by operators for this event.");
    }
    ...
    for (const event of todayEvents) {
      ...
      for (const participant of participants) {
        if (participant.reminderSent) {
          console.log(
            `Reminder already sent for participant: ${
        ...
        const timeslotField = participant.participantFields.find(
        ...
        if (timeslotField) {
          ...
          const formattedDate = moment(`${datePart}`, "DD MM YYYY").format(
            "YYYY-MM-DD"
          );
          ...
          if (timeslot.isBetween(now, oneHourLater, null, "[]")) {
            console.log(
              `Sending reminder for participant: ${
                participant.participantFields.find(
                  field => field.fieldName === "Name"
                ).fieldValue
              }`
            ...
            const mobileNumber = participant.participantFields.find(
            ...
            const message = `Hello ${
              ...
              event.eventName
            } scheduled at ${timeslot.format("HH:mm")} today.
            
            Powered by JoiningEnds.`;
...
const sendReminderEmail = async (event, participant, emailConfigData) => {
  const participantName = participant.participantFields.find(
    field => field.fieldName === "Name"
  ).fieldValue;
  const participantEmail = participant.participantFields.find(
    field => field.fieldName === "Email"
  ).fieldValue;
  const timeslot = participant.participantFields.find(
    field => field.fieldName === "timeslot"
  ).fieldValue;
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <p>This is a reminder that the event <strong>${
        event.eventName
      }</strong> is scheduled to start at <strong>${
      event.startTime
    }</strong> on <strong>${event.eventDate}</strong>.</p>
...
exports.updateRegistration = async (req, res) => {
  try {
    const { id } = req.params;
    const { participantFields } = req.body;

    if (!Array.isArray(participantFields) || participantFields.length === 0) {
      return res
        .status(400)
        .json({ message: "Invalid participant fields data" });
    }

    const updatedRegistration = await Registration.findByIdAndUpdate(
      id,
      { $set: { participantFields: participantFields } },
