  ...
  try {
    ...
    if (eventType === "1" || eventType === "2") {
      const sessionDuration =
        sessionTimePerCounterHours * 60 + sessionTimePerCounterMinutes;
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    if (updateData.eventType !== existingEvent.eventType) {
      // Generate new URL based on the updated event type
      updatedEvent.url =
        updateData.eventType === "0"
          ? `http://localhost:5001/normalEvent/${updatedEvent._id}`
          : `http://localhost:5001/eventTimeSlot/${updatedEvent._id}`;

      console.log("Event type changed. New URL generated:", updatedEvent.url);

      // Save the updated event with the new URL
