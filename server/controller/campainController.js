  ...
  try {
    ...
    if (existingCampaign) {
      return res
        ...
        .json({
          error:
            "A campaign already exists for this profile at the scheduled time",
        });
    }
    ...
    const upcomingCampaign = await Campaign.findOne({
      profile,
      scheduleTime: { $gte: now, $lte: thirtyMinutesFromNow },
    });
    if (upcomingCampaign) {
      return res
        .status(400)
        .json({
          error:
            "A campaign is already scheduled in the next 30 minutes for this profile",
        });
    }
    ...
    for (const campaign of allCampaigns) {
      const whatsappMessage = await WhatsAppMessage.findOne({
        campainid: campaign._id,
      });
      if (whatsappMessage) {
        const maxIntervalM = section.count * whatsappMessage.maxIntervalMs;
        const bufferTime = 30 * 60 * 1000; // 30 minutes in milliseconds
        const campaignEndTime =
          new Date(campaign.scheduleTime).getTime() + maxIntervalM + bufferTime;
        ...
        if (new Date(scheduleTime).getTime() < campaignEndTime) {
          ...
          return res
            .status(400)
            .json({
              error: `An ongoing campaign exists. You can create a new campaign after ${formattedEndDate}`,
            });
        }
      }
    }
...
exports.updateCampaign = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, client, contractGroup, profile, successCount, failureCount } =
      req.body;
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    if (result.deletedCount === 0) {
      return res
        .status(404)
        .json({ error: "No campaigns found for the specified clientId" });
    }
