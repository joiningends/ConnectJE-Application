...
exports.createRole = async (req, res) => {
  try {
    ...
    res.status(201).json({
      message:
        "User created successfully. A password reset link will be sent via email shortly.",
      newRole,
    });
...
exports.editRole = async (req, res) => {
  try {
    ...
    existingRole.Email = Email || existingRole.Email;
    existingRole.Mobile = Mobile || existingRole.Mobile;
...
exports.getAllRolesbyrole = async (req, res) => {
  ...
  try {
    ...
    if (roles.length > 0) {
      ...
      res
        .status(404)
        .json({ message: "No roles found for the given criteria" });
    }
  } catch (error) {
