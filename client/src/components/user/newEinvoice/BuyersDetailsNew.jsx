    ...
    if (validate()) {
      ...
      try {
        ...
        const postData = {
          ...
          place_of_supply: selectedStateCode,
          state_code: selectedStateCode,
        };

        console.log(postData);
        await axios.post("http://localhost:5001/api/v1/eibuyer", postData);
        toast.success("Buyer details submitted successfully!");
        navigate("/BuyersDetailsTable");
      } catch (error) {
        toast.error("Failed to submit buyer details.");
      } finally {
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-end space-x-4">
            <button
              ...
              onClick={() => navigate("/BuyersDetailsTable")}
            >
              Cancel
            </button>
            <button
              type="submit"
              className={`bg-[#B197FC] hover:bg-[#9f85ff] text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ${
            ...
            >
              {isSubmitting ? (
                <div className="flex items-center justify-center">
                  <div className="spinner-border animate-spin inline-block w-4 h-4 border-2 rounded-full border-t-white mr-2"></div>
                  Submitting...
                </div>
              ) : (
                "Submit"
...
export default BuyersDetailsNew;
