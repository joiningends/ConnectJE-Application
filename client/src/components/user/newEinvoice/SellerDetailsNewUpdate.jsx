...
const SellerDetailsNewUpdate = () => {
    ...
    const fetchSellerDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5001/api/v1/eiseller/${id}`
        );
        ...
        if (response.status === 200) {
          ...
          setSellerDetails({
            ...
            state: states.find(state => state.code === data.state_code) || "",
          });
        } else {
  ...
  const handleCancel = () => {
    navigate("/SellerTable"); // Adjust the route as needed
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-8 flex justify-between">
            <button
              ...
              className="bg-gray-500 text-white font-bold py-2 px-8 rounded-md hover:bg-gray-600 transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="bg-[#B197FC] text-white font-bold py-2 px-8 rounded-md hover:bg-[#9F7AEA] transition-colors"
...
export default SellerDetailsNewUpdate;
