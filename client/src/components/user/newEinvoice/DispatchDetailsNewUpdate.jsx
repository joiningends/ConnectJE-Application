...
const DispatchDetailsNewUpdate = () => {
  ...
  // Fetch dispatch details when the component mounts
  useEffect(() => {
    const fetchDispatchDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5001/api/v1/eidispatch/${id}`
        );
        setDispatchDetails(response.data); // Populate form with fetched data
      } catch (error) {
  ...
  const validate = () => {
    ...
    } else if (dispatchDetails.pincode.toString().length !== 6) {
  ...
  const handleSubmit = async e => {
    ...
    if (validate()) {
      ...
      try {
        ...
        // Make the PUT request to update the dispatch details
        await axios.put(
          `http://localhost:5001/api/v1/eidispatch/${id}`,
          payload
        );
        ...
        toast.error("Failed to update dispatch details. Please try again.");
      } finally {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end space-x-4">
            ...
            <button
              ...
              className={`bg-[#B197FC] text-white px-6 py-2 rounded-md flex items-center ${
...
export default DispatchDetailsNewUpdate;
