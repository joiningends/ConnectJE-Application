...
export default function ScannerOptometrist() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5001/api/v1/eventmodel/optomatics/${whatsappUserId}`
      );
  ...
  const verifyRegistration = async (registrationId, eventId) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5001/api/v1/register/opt/${registrationId}/${eventId}/${whatsappUserId}`
      );
      ...
      // Navigate to the TestResult page after successful scan
      navigate(`/TestResult/${registrationId}/${eventId}`);
    } catch (error) {
      if (
        error.response &&
        error.response.data ===
          "QR code has already been scanned by the operator."
      ) {
  ...
  const handleManualEntry = async (registrationId, eventId) => {
    try {
      const whatsappUserId = localStorage.getItem("whatsappuserId");
      await verifyRegistration(registrationId, eventId);
      // Refresh the attendee list after manual entry
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, nameSearch, phoneSearch, attendees]);
